...
$(document).ready(function()
{
	...
	$('td[type="phone"],.phone,#phone_work,#phone_other,#phone_mobile,.asterisk_phoneNumber').each(function()
	{
        ...
        if(phoneNr.length > 1 && !/(class="phone"|id="?#phone)/.test($(this).html()))
		{
			...
			/*if( AST_UserExtention ) {
				$(this).append('&nbsp;&nbsp;<img onclick="changeCss($(this));" src="custom/modules/Asterisk/include/call_active.gif" class="asterisk_placeCall" value="anrufen" style="cursor: pointer;transition: all .2s ease-in-out;"/>&nbsp;');	
			}
			else {
			...
			$('.asterisk_placeCall', this).click(function()
			{
			    ...
			    $.ajax({
					url:'index.php?entryPoint=AsteriskCallCreate',
					data: {phoneNr : phoneNr, contactId: contactId},
					type:'GET',
					 success: function(result){
					 	$('body').append(result);
					 	setTimeout(function(){
					 		$('#iframe-for-call').remove();
					 	},2000)
				    }
				})
			});
