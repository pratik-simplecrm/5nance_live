...
$aColumns = array(unique_customer_code_c, first_name, last_name, email_address, phone_mobile, gateway_c, campaign_type_c, user_name, product_interest_c, product_sub_type_interest_c, date_entered, sales_stage_c, next_call_planning_comments_c, user_allocation_date_c, custom_date_modified_c, utm_content_c, campaign_c, source_c,drop_stage_c);
//$whereColumns = array('c.id', 'c.first_name', 'c.last_name', 'cc.unique_customer_code_c', 'ea.email_address', 'c.phone_mobile', 'cc.gateway_c', 'cc.campaign_type_c', 'u.user_name', 'cc.product_interest_c', 'cc.product_sub_type_interest_c', 'c.date_entered', 'cc.sales_stage_c', 'cc.next_call_planning_comments_c', 'cc.user_allocation_date_c', 'cc.disposition_c', 'cc.qparam_c','cc.custom_date_modified_c','cc.utm_content_c', 'cc.campaign_c', 'cc.source_c', 'cc.drop_stage_c');

$whereColumns = array('cc.unique_customer_code_c', 'c.first_name', 'c.last_name','ea.email_address', 'c.phone_mobile', 'cc.gateway_c', 'cc.campaign_type_c', 'u.user_name', 'cc.product_interest_c', 'cc.product_sub_type_interest_c', 'cc.utm_content_c',  'cc.campaign_c', 'cc.source_c', 'cc.drop_stage1_c', 'cc.drop_stage1_c','cc.drop_stage1_c','cc.drop_stage1_c','cc.drop_stage1_c','cc.drop_stage1_c','cc.drop_stage1_c','cc.drop_stage1_c','cc.drop_stage1_c','cc.drop_stage1_c','cc.drop_stage1_c','cc.sales_stage_c', 'cc.next_call_planning_comments_c', 'cc.disposition_c');
    ...
    for ($i = 0; $i < intval($_GET['iSortingCols']); $i++) {
        if (!empty($whereColumns[intval($_GET['iSortCol_' . $i])])) {
            if ($_GET['bSortable_' . intval($_GET['iSortCol_' . $i])] == "true") {
                $sOrder .= $whereColumns[intval($_GET['iSortCol_' . $i])] . " " . mysql_real_escape_string($_GET['sSortDir_' . $i]) . ", ";
            }
        }
    }
...
if (!empty($gateway)) {
    ...
    $gateway = '';
				...
				$gateway = "and cc.gateway_c in ('$gateway')";
...
//$sQuery = "SELECT SQL_CALC_FOUND_ROWS DISTINCT c.id,c.id as id, c.first_name as first_name, c.last_name as last_name, cc.leadid_c as unique_customer_code_c, ea.email_address as email_address, c.phone_mobile as phone_mobile, cc.gateway_c as gateway_c, cc.campaign_type_c as campaign_type_c, u.user_name as user_name, cc.product_interest_c as product_interest_c, cc.product_sub_type_interest_c as product_sub_type_interest_c, c.date_entered as date_entered, cc.sales_stage_c as sales_stage_c, cc.next_call_planning_comments_c as next_call_planning_comments_c, cc.user_allocation_date_c as user_allocation_date_c, cc.disposition_c as latest_disposition,cc.qparam_c as publisher_subid,cc.custom_date_modified_c as date_modified, cc.utm_content_c as utm_content_c,cc.campaign_c as campaign_c, cc.source_c as source_c, cc.drop_stage_c as drop_stage, cc.rejection_reason_c as rejection_reason, cc_city.name as city_name, cc.age_of_the_user_c as age_user , cc.annual_income_c as annual_income, cc.occupational_details_c as occupational_details FROM contacts c LEFT JOIN contacts_cstm cc ON c.id = cc.id_c join city cc_city on cc_city.id = cc.city join users u on u.id = c.assigned_user_id LEFT JOIN email_addr_bean_rel eabl  ON eabl.bean_id = c.id AND eabl.bean_module = 'Contacts' and eabl.primary_address = 1 and eabl.deleted=0 LEFT JOIN email_addresses ea ON (ea.id = eabl.email_address_id) where " . $sWhere . " $date_filter  c.deleted=0 AND u.user_name !='poorti' AND cc.gateway_c!=''" . $sOrder . $sLimit . "";



$sQuery = "SELECT SQL_CALC_FOUND_ROWS DISTINCT c.id,c.id as id, c.first_name as first_name, c.last_name as last_name, cc.leadid_c as unique_customer_code_c, ea.email_address as email_address, c.phone_mobile as phone_mobile, cc.gateway_c as gateway_c, cc.campaign_type_c as campaign_type_c, u.user_name as user_name, cc.product_interest_c as product_interest_c, cc.product_sub_type_interest_c as product_sub_type_interest_c, c.date_entered as date_entered, cc.sales_stage_c as sales_stage_c, cc.next_call_planning_comments_c as next_call_planning_comments_c, cc.user_allocation_date_c as user_allocation_date_c, cc.disposition_c as latest_disposition,cc.qparam_c as publisher_subid,cc.custom_date_modified_c as date_modified, cc.utm_content_c as utm_content_c,cc.campaign_c as campaign_c, cc.source_c as source_c, cc.drop_stage1_c as drop_stage, cc.rejection_reason_c as rejection_reason, city_t.name as city_name, cc.loan_dob_c as age_user , cc.loan_income_c as annual_income, cc.loan_occupation_c as occupational_details, cc.intent_c as intent_c, cc.marital_status_c as marital_status FROM contacts c LEFT JOIN contacts_cstm cc ON c.id = cc.id_c LEFT JOIN city as city_t on city_t.id = cc.city_c join users u on u.id = c.assigned_user_id LEFT JOIN email_addr_bean_rel eabl  ON eabl.bean_id = c.id AND eabl.bean_module = 'Contacts' and eabl.primary_address = 1 and eabl.deleted=0 LEFT JOIN email_addresses ea ON (ea.id = eabl.email_address_id) where " . $sWhere . " $date_filter  c.deleted=0 AND u.user_name !='poorti' AND cc.gateway_c!=''" . $sOrder . $sLimit . "";
...
$sQuery = "SELECT COUNT(c." . $sIndexColumn . ") FROM contacts c LEFT JOIN users u on u.id = c.assigned_user_id where c.deleted=0 AND u.user_name !='poorti' ";
...
//      }
    ...
    $row[] = w1250_to_utf8($contact_row['campaign_type_c']);
    $row[] = $contact_row['user_name'];
    $row[] = w1250_to_utf8($contact_row['product_interest_c']);
    $row[] = w1250_to_utf8($contact_row['product_sub_type_interest_c']);

    $row[] = w1250_to_utf8($contact_row['utm_content_c']);
    $row[] = w1250_to_utf8($contact_row['campaign_c']);
    $row[] = w1250_to_utf8($contact_row['source_c']);


$row[] = str_replace("_"," ",$contact_row['drop_stage']);
$row[] = w1250_to_utf8($contact_row['rejection_reason']);
$row[] = $contact_row['city_name'];

if(!empty($contact_row['age_user']))
{
    $birthdate = new DateTime($contact_row['age_user']);
...
$row[] = $contact_row['annual_income'];
$row[] = w1250_to_utf8($contact_row['occupational_details']);
$row[] = $contact_row['intent_c'];

$row[] = w1250_to_utf8($contact_row['marital_status']);




    $row[] = w1250_to_utf8($contact_row['publisher_subid']);
    ...
    $row[] = str_replace("_"," ",$contact_row['sales_stage_c']);
    $row[] = w1250_to_utf8($contact_row['next_call_planning_comments_c']);
	//utf8_encode($contact_row['next_call_planning_comments_c']);//preg_replace('/[^A-Za-z0-9\-]/', '',$contact_row['next_call_planning_comments_c']);
