...
$aColumns = array(unique_customer_code_c, first_name, last_name, email_address, phone_mobile, gateway_c, no_of_attempts_c,campaign_type_c, user_name, product_interest_c, product_sub_type_interest_c, date_entered, sales_stage_c, next_call_planning_comments_c, user_allocation_date_c, date_modified, utm_content_c,campaign_c,source_c);
$whereColumns = array('c.id', 'c.first_name', 'c.last_name', 'cc.unique_customer_code_c', 'ea.email_address', 'c.phone_mobile', 'cc.gateway_c','cc.no_of_attempts_c', 'cc.campaign_type_c', 'u.user_name', 'cc.product_interest_c', 'cc.product_sub_type_interest_c', 'c.date_entered', 'cc.sales_stage_c', 'cc.next_call_planning_comments_c', 'cc.user_allocation_date_c', 'cc.disposition_c', 'cc.qparam_c', 'c.date_modified', 'cc.utm_content_c','cc.campaign_c','cc.source_c','cc.related_corporate_account_c','cc.partner_comments_c');
$prowhereColumns=array('c.id', 'c.first_name', 'c.last_name', 'cc.unique_customer_code_c', 'ea.email_address', 'c.phone_mobile', 'cc.gateway_c','pc.no_of_attempts_c', 'cc.campaign_type_c', 'u.user_name', 'pc.product_interest_c', 'pc.product_sub_type_interest_c', 'p.date_entered', 'pc.sales_stage_c', 'pc.next_call_planning_comments_c', 'cc.user_allocation_date_c', 'pc.disposition_c', 'cc.qparam_c', 'p.date_modified', 'cc.utm_content_c','cc.campaign_c','cc.source_c','cc.related_corporate_account_c','cc.partner_comments_c');
...
if (isset($_GET['iSortCol_0'])) {
    $pOrder = " ORDER BY  ";
    for ($i = 0; $i < intval($_GET['iSortingCols']); $i++) {
        if (!empty($prowhereColumns[intval($_GET['iSortCol_' . $i])])) {
            if ($_GET['bSortable_' . intval($_GET['iSortCol_' . $i])] == "true") {
                $pOrder .= $prowhereColumns[intval($_GET['iSortCol_' . $i])] . " " . addslashes($_GET['sSortDir_' . $i]) . ", ";
            }
        }
    }

    $pOrder = substr_replace($pOrder, "", -2);
    if ($pOrder == "ORDER BY") {
        $pOrder = "";
    }
}
...
if ($_GET['sSearch'] != "") {
    $pWhere = "(";
    for ($i = 0; $i < count($prowhereColumns); $i++) {

        $pWhere .= $prowhereColumns[$i] . " LIKE '%" . addslashes($_GET['sSearch']) . "%' OR ";
    }
//		$sWhere .= " (SELECT CONCAT( COALESCE(u.first_name,''), ' ', COALESCE(users.last_name,'') ) as rn from users where u.reports_to_id=users.id) LIKE '%".mysql_real_escape_string( $_GET['sSearch'] )."%' OR ";
    $pWhere = substr_replace($pWhere, "", -3);
    $pWhere .= ') AND';
}
...
for ($i = 0; $i < count($prowhereColumns); $i++) {
    if ($_GET['bSearchable_' . $i] == "true" && $_GET['sSearch_' . $i] != '') {
        ...
        $pWhere .= $prowhereColumns[$i] . " LIKE '%" . addslashes($_GET['sSearch_' . $i]) . "%' ";
    }
}
/*
 * SQL queries
...
$productinterest = $_GET['productinterest'];
$no_of_attempts = $_GET['no_of_attempts'];
$salesadvisors = $_GET['salesadvisors'];
...
//		}
    ...
    $row[] = $contact_row['no_of_attempts_c'];
    ...
    $row[] = $contact_row['publisher_subid'];
     $row[] = $contact_row['partner_name'];
      $row[] = $contact_row['partner_comment'];
