...
class AOR_Report extends Basic {
    ...
    function build_report_html($offset = -1, $links = true, $group_value = '', $tableIdentifier = '', $extra = array()){
        ...
        $file = fopen("query3.txt","w");
        fwrite($file, print_r($report_sql,true));
        fclose($file);
    ...
    function getTotalHTML($fields,$totals){
        ...
        foreach($fields as $label => $field){
            ...
            if($field['total']){
                ...
                $totalLabel = "<b>Total</b> " .$field['label'];
    ...
    function build_report_query($group_value ='', $extra = array()){
		...
		$GLOBALS['log']->fata(print_r($query_array,true));
    ...
    function build_report_query_where($query = array()){
        ...
        if($beanList[$this->report_module]){
            ...
            while ($row = $this->db->fetchByAssoc($result)) {
                ...
                //~ echo "<br>";echo "<br>";
				//~ echo $condition->value;echo "<br>";echo "<br>";echo "<br>";
				//~ echo $condition->value_type;echo "<br>";echo "<br>";
				//~ echo $condition->operator;echo "<br>";echo "<br>";
...
//                            $table_alias = 'opportunities:assigned_user_link:SecurityGroups' ;
                    ...
                    switch ($condition->value_type) {
                        ...
                        {
							//$GLOBALS['log']->fatal($data['type'],"Data type");
							...
							{
								$date = '"'.date('Y-m-d',strtotime('+480 minutes',strtotime($params))).'%'.'"';
							$value = $field.' '.'LIKE('."$date".')';
							}
							$query['where'][] = $value;
                            $where_set = true;
							//$GLOBALS['log']->fatal($value,"Value in datetime");
							}
							else if($data['type'] =='datetimecombo')
							{
								$date1=$condition->value;
								if($condition->condition_order != '0')
							{
							$date = '"'.date('Y-m-d',strtotime('+480 minutes',strtotime($params))).'%'.'"';
							//$date = '"'.date('Y-m-d',strtotime('-5 hours, -30 minutes',strtotime($date1))).'%'.'"';
									$end_date = date("Y-m-d H:i:s", strtotime("+18 hours, +30 minutes", strtotime($date1)));
									$start_date = date("Y-m-d H:i:s", strtotime("-1 day", strtotime($end_date)));
									$end_date = date("Y-m-d H:i:s", strtotime("-1 second", strtotime($end_date)));
									$value =  'AND'.' '.$field.">='".$start_date ."' AND ".$field."<='".$end_date . "'";
							}
							else
							{
									//$date = '"'.date('Y-m-d',strtotime('-5 hours, -30 minutes',strtotime($date1))).'%'.'"';
									...
									$start_date = date("Y-m-d H:i:s", strtotime("-1 day", strtotime($end_date)));
									$end_date = date("Y-m-d H:i:s", strtotime("-1 second", strtotime($end_date)));
									$value =  $field.">='".$start_date ."' AND ".$field."<='".$end_date . "'";
							}
									$query['where'][] = $value;
						...
						} else if($condition->operator=='Greater_Than_or_Equal_To' || $condition->operator=='Less_Than' || $condition->operator=='Greater_Than') // Added by Shakeer on 21 Jan 2017
                        {
							if($data['type'] =='datetime' || $data['type'] =='datetimecombo' )
							{
								if (preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/",$condition->value))
								{
									$condition->value = date("Y-m-d H:i:s" , strtotime("-5 hours, -30 minutes" , strtotime($condition->value)));
								}
							}
							$value = "'" . $this->db->quote($condition->value) . "'";
								...
								if($data['type'] =='datetime' || $data['type'] =='datetimecombo' )
							{
								if (preg_match("/^[0-9]{4}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/",$condition->value))
								{
									$condition->value = date("Y-m-d 18:29:59" , strtotime($condition->value));
								}
							}
							$value = "'" . $this->db->quote($condition->value) . "'";
						}else {
                    ...
                    if(!$where_set) {
                        if ($condition->value_type == "Period") {
                          ...
                          $date = date("Y-m-d H:i:s", strtotime("-5 hours, -30 minutes, -1 seconds", strtotime($date)));
							$value = '"' . date("Y-m-d H:i:s", strtotime("-5 hours, -30 minutes, 0 seconds", strtotime($value))) . '"';
