...
class Email extends SugarBean {
	...
	function email2Send($request) {
		...
		/* satisfy basic HTML email requirements */
                        //For adding background image to email template changes by Roshan Sarode 9-5-18 start
       
        $this->name = $request['sendSubject'];
        $GLOBALS['log']->fatal($_REQUEST['get_template']."email templated");
        if ($_REQUEST['get_template'] == '73abb972-2a24-07a9-284c-5add7f8786bc') {
        	$GLOBALS['log']->fatal(print_r($this,true)."email background image");
            $req_desc = str_replace('&lt;style&gt;', '<style>.banner-sec-img{background-image: url(http://bs.m3c.in/images/5nance/Noncontactable/Non_Cont_bg.png) !important;background-repeat:no-repeat;}', $request['sendDescription']);
        } else {
            $req_desc = $request['sendDescription'];
        }


        $this->description_html = '&lt;html&gt;&lt;body&gt;' . $req_desc . '&lt;/body&gt;&lt;/html&gt;';

                        //For adding background image to email template changes by Roshan Sarode 9-5-18 end
