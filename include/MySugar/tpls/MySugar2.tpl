...
<script>
    {literal}
            if ($(".dashboardTabList .active a span").html() == "My Leads")
            {
                if ($("#Fresh_Calls_Till_date").length == 1) {

                  var did= $("#Fresh_Calls_Till_date").closest('tr').find('.refresh_icon').attr('id');
                  did2 = (did.substring(did.lastIndexOf("_"))).replace("_", "");
              
                  SUGAR.mySugar.retrieveDashlet(did2);
                }
