    ...
    SUGAR.mySugar = function() {
        ...
        return {
            ...
            addDashlet: function(id, type, type_module) {
                ...
                var success = function(data) {
                    var respid =$.trim(data.responseText);    //data.responseText in that variable space is coming that's why user was not able to add dashlets so that's why added trim function for the variable- Changes by Roshan Sarode 13-11-2017
                    
                    colZero = document.getElementById('col_' + activeTab + '_0');
                    newDashlet = document.createElement('li');
                    newDashlet.id = 'dashlet_' + respid;
                    newDashlet.className = 'noBullet active';
                    newDashlet.innerHTML = '<div style="position: absolute; top: -1000px; overflow: hidden;" id="dashlet_entire_' + respid + '"></div>';
                    colZero.insertBefore(newDashlet, colZero.firstChild);
                    var finishRetrieve = function() {
                        dashletEntire = document.getElementById('dashlet_entire_' + respid);
                        dd = new ygDDList('dashlet_' + respid);
                        dd.setHandleElId('dashlet_header_' + respid);
                        dd.dashletID = respid;
                        ...
                        var anim = new YAHOO.util.Anim('dashlet_entire_' + respid, {
                            height: {
                                to: dashletRegion.bottom - dashletRegion.top
                            }
                        }, .5);
                        anim.onComplete.subscribe(function() {
                            document.getElementById('dashlet_entire_' + respid).style.height = '100%';
                        });
                        anim.animate();
                        newLayout = SUGAR.mySugar.getLayout(true);
                        SUGAR.mySugar.saveLayout(newLayout);
                    ...
                    SUGAR.mySugar.retrieveDashlet(respid, url, finishRetrieve, true);
                }
